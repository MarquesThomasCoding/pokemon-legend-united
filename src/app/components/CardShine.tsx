...
export const CardShine = ({ pokemon, show, initialReverse, canSelect, setTypes, setRegion}: { pokemon: PokemonItem, show: boolean, initialReverse?: boolean, canSelect?: boolean, setTypes: string, setRegion: string}) => {
    ...
    return (
        ...
        style={{display: (setTypes==="all" || setTypes===pokemon.types[0].type.name || setTypes===pokemon.types[1]?.type.name) ? "": "none !important"}}
            ...
            <div className={"[backface-visibility:hidden] absolute top-0 left-0 flex justify-between w-full h-full rounded-xl bg-gradient-to-r " + typesGradients[pokemon.types[0].type.name] + " cursor-pointer shadow-md shadow-gray-500"}>
                <div>
                ...
                <Link href="/pokemon/[id]" as={`/pokemon/${pokemon.id}`} className="p-2 rounded-se-xl rounded-es-xl h-fit hover:bg-gray-700 hover:bg-opacity-50 transition-all duration-150"><EyeIcon /></Link>
